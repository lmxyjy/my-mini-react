...
let deletions = null; //----------------------------scheduler 阶段----------------------------
...
requestIdleCallback(workLoop); //----------------------------reconcile 阶段----------------------------
...
function reconcile(wipFiber, elements) {
  ...
  while (index < elements.length || oldFiber != null) {
    ...
    const sameType = oldFiber && element && element.type === oldFiber.type; //type没有改变，意味着标签名不变，打上更新标识
...
function createDom(fiber) {
  ...
  const dom =
    type === TEXT_ELEMENT
      ...
      : document.createElement(type); // 添加属性
...
function updateDom(dom, prevProps, nextProps) {
  ...
  Reflect.ownKeys(prevProps)
    ...
    .forEach((name) => (dom[name] = "")); //添加新的属性

  Reflect.ownKeys(nextProps)
    .filter(isProperty)
    .filter(isNew(prevProps, nextProps))
    .forEach((name) => (dom[name] = nextProps[name])); //移除不需要的监听事件
