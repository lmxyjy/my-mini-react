...
const isEvent = (key) => key.startsWith("on"); //判断是否为需要的属性

const isProperty = (key) => key !== "children" && !isEvent(key); //判断是否为变化的属性

const isNew = (prev, next) => (key) => prev[key] !== next[key]; //判断是否为prev中与next不同的属性

const isGone = (prev, next) => (key) => !(key in next); //根据传入的fiber对象创建dom元素
...
function updateDom(dom, prevProps, nextProps) {
  ...
  Reflect.ownKeys(prevProps)
    ...
    .forEach((name) => (dom[name] = "")); //设置属性

  Reflect.ownKeys(nextProps)
    .filter(isProperty)
    .filter(isNew(prevProps, nextProps))
    .forEach((name) => (dom[name] = nextProps[name])); //移除不需要的或者变化的事件
...
function commitRoot() {
  //删除不需要的dom
  deletions.forEach(commitWork); //渲染其他的dom
...
function commitWork(fiber) {
  ...
  const domParent = fiber.parent.dom; //处理不同的effectTag
